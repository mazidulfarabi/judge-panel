...
3. **Assign**: Random N teams to one judge, assign all teams to one judge, or auto-distribute (each unassigned team → one judge, round-robin, up to N per judge).
4. **Manage**: Delete judges or teams (single or all teams); delete all assignments at once; remove a single assignment (and its marks); clear scores only (keeps assignment so the judge can mark again).
